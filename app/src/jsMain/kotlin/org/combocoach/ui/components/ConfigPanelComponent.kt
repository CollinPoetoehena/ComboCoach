...
/**
 * Creates and manages the configuration panel
 */
object ConfigPanelComponent {
    
    fun create(config: TrainingConfiguration, isExpanded: Boolean, onToggle: () -> Unit): HTMLElement {
        return document.createElement("div").apply {
            ...
            // Collapsible content
            if (isExpanded) {
                appendChild(createContent(config))
            }
        } as HTMLElement
    }
    ...
    private fun createHeader(onToggle: () -> Unit): HTMLElement {
        return document.createElement("div").apply {
            ...
            """
            querySelector("#config-toggle-btn")?.addEventListener("click", { onToggle() })
        } as HTMLElement
    }
    
    private fun createContent(config: TrainingConfiguration): HTMLElement {
        return document.createElement("div").apply {
    ...
    /**
     * Reads the current configuration values from the DOM inputs
     */
